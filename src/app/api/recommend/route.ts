...
export async function POST(req: NextRequest) {
  ...
  const prompt = `
    ...
    Here are the user answers for their laptop use cases:
    ${JSON.stringify(data)}.
    Recommend good laptops for the selected use cases.
    Search for them on google.
    Give 2 shoppinglinks to the recommended laptops on Amazon using these urls https://www.amazon.com/s?k={laptop_name} and on smartprix using this url https://www.smartprix.com/products/?q={laptop_name}. 
  ...
  return NextResponse.json(
    {
      aai:  result.response.text().replace("```json", "").replace("```", ""),
